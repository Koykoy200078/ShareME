...
function loadMysqlClient() {
	try {
		...
		throw new Error(`mysql2 is required to run rollback: ${error.message}`)
	}
}

function assertRollbackConfirmed() {
	const hasCliFlag = process.argv.includes('--yes')
	const hasEnvFlag = String(process.env.EVENTSCORER_DB_ROLLBACK_CONFIRM || '').toUpperCase() === 'YES'

	if (hasCliFlag || hasEnvFlag) {
		return
	}

	throw new Error('Rollback confirmation required. Re-run with --yes or EVENTSCORER_DB_ROLLBACK_CONFIRM=YES')
}

async function run() {
	assertRollbackConfirmed()

	const mysql = loadMysqlClient()
	const config = buildDatabaseConfig()
	const sqlPath = path.join(__dirname, 'sql', 'eventscorer-rollback.sql')
	const rollbackSql = await fs.readFile(sqlPath, 'utf8')
	...
	try {
		...
		console.log(`[eventscorer:rollback] Tables dropped from ${config.host}:${config.port}/${config.database}`)
	} finally {
...
run().catch((error) => {
	console.error('[eventscorer:rollback] Failed:', error.message)
	process.exitCode = 1
})
