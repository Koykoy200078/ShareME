...
// Regular API calls go through the local /api route handler proxy.
const API = '/api'
...
function getUploadBase(): string {
	...
	if (typeof window === 'undefined') return `http://localhost:${backendHttpPort}`

	// Use backend HTTP fallback port when page is served over HTTP.
	const targetPort = window.location.protocol === 'http:' ? backendHttpPort : backendPort
...
export function useUpload(onDone: () => void, notify: (m: string, t: NotifType) => void) {
	...
	const startUpload = useCallback(
		async (files: TaggedFile[]) => {
			if (!files.length) {
				notify('Select files first', 'error')
				return
			}
			cancelRef.current = false
			setState('uploading')
			...
			async function uploadOne(file: TaggedFile, retry = 0): Promise<void> {
				...
				const rawName = isFolder ? file.webkitRelativePath || file.relativePath || file.name : file.name
				...
				await new Promise<void>((resolve) => {
					...
					xhr.onerror = () => {
						xhrs.current = xhrs.current.filter((x) => x !== xhr)
						if (retry < MAX_RETRIES && !cancelRef.current) setTimeout(() => uploadOne(file, retry + 1).then(resolve), 2000 * (retry + 1))
						else {
							failed++
							uploadedBytes += file.size
							resolve()
