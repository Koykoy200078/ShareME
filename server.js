...
function getLocalIPAddress() {
	const configuredIP = (process.env.SERVER_IP || '').trim()
	if (configuredIP && isLocalIPv4Address(configuredIP)) return configuredIP
...
async function getDiskSpace() {
	return new Promise((resolve) => {
		...
		exec(`powershell -NoProfile -Command "[System.IO.DriveInfo]::new('${drive}') | Select-Object AvailableFreeSpace,TotalSize | ConvertTo-Json"`, (err, stdout) => {
...
function getPrinterColorEnabled(printerName) {
	return new Promise((resolve) => {
		...
		exec(`powershell -NoProfile -Command "(Get-PrintConfiguration -PrinterName '${safe}' -ErrorAction SilentlyContinue).Color"`, (err, stdout) => {
...
function setPrinterColorEnabled(printerName, enabled) {
	return new Promise((resolve) => {
		...
		exec(`powershell -NoProfile -Command "Set-PrintConfiguration -PrinterName '${safe}' -Color ${val} -ErrorAction SilentlyContinue"`, (err) => {
...
server.listen(PORT, '0.0.0.0', () => {
	...
	console.log(`[HTTP]  Backend running at http://${localIP}:${PORT}`)
})
...
// (WebSocket upgrade handlers moved to the server configuration loop above)
